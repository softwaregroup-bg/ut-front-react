export const isIPValid = (IPString) => {
    const rgx = /^(([0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])\.){3}([0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])$/;
...
export function compareIpRanges(ipFrom, ipTo) {
    if (isIPValid(ipFrom) && isIPValid(ipTo)) {
        const ipFromDec = parseIPToDecimal(ipFrom);
        const ipToDec = parseIPToDecimal(ipTo);

...
class Documents extends Component {
    ...
    componentWillReceiveProps(nextProps) {
        if (nextProps.selectedFilter === 'archived' && nextProps.documentArchived.get('requiresFetch')) {
            this.fetchArchivedDocs(nextProps.objectId, nextProps.objectType, nextProps.documentArchived.get('requiresFetch'), nextProps.documentArchived.get('isLoading'), nextProps.identifier);
        }
    }

    fetchArchivedDocs(objectId, objectType, requiresFetch, isLoading, identifier) {
        if (objectId && objectType && requiresFetch && !isLoading) {
            this.props.fetchArchivedDocuments(objectId, objectType, identifier);
        }
    }

...
class CompareGrid extends Component {
    renderDataGrid(data, type) {
        return data.get('data').map((values, index) => {
            ...
            return <TitledContentBox key={index} externalContentClasses={styles.titledBoxBody} externalHeaderClasses={styles.titleBoxTitle} title={title}>
    ...
    render() {
        ...
        return (
            <div>
                <Accordion title={data.get('title')} marginBottom={false} externalBodyClasses={styles.accordionBody} externalTitleClasses={styles.accordionTitle} className={styles.accordion} collapsed={!data.get('isOpen')}>

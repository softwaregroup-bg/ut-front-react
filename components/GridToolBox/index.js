        ...
        case filterElementTypes.dateTimePickerBetween:
          if (filter.defaultValue) {
            if (!filter.initialValue) {
              ...
              hasValue =
                filter.defaultValue.from !== filter.initialValue.from ||
                filter.defaultValue.to !== filter.initialValue.to;
            }
          }
          break;
    ...
    const el = (
      ...
      >
        ...
        {tooltipContent.length ? (
          <div className={style.advancedSearchPopOver}>{tooltipContent}</div>
        ) : null}
      </div>
    );
    ...
    return (
      ...
      >
        ...
        <div className={classnames(style.pullRight, style.tableCell)}>
          ...
          >
            {filterElements.map((el, i) => {
              const incrementNum =
                el.type === filterElementTypes.datePickerBetween ||
                el.type === filterElementTypes.dateTimePickerBetween
                  ? 2
                ...
                return (
                  filter && (
                    <div
                      ...
                      style={el.styles}
                    >
                      <div className={style.minWidthed}>{filter}</div>
                    </div>
                  )
                );
              }
            ...
            {showSearchBtn && (
              <div
                key="searchBtn"
                className={classnames(style.toolbarElement, style.tableCell)}
              >
        ...
        {filterActionElements && (
          <div className={style.filterActionWrap}> {filterActionElements} </div>
        )}
      </div>
    );
  }
          ...
          <div className={classnames(style.table, style.fixedHeight)}>
            {this.props.actionButtonElements.map((el, i) => {
              return (
                <div
                  key={i}
                  className={classnames(style.tableCell, style.spacer)}
                >
